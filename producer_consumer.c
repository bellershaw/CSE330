...
printk(KERN_INFO "Added task %d w/ user %d to buffer", p->pid, p->cred->uid.val);
...
printk(KERN_INFO "%s consumed Item#-%d on buffer index: %d PID:%d Elapsed Time- %ld", current->comm, 1, 0, temp->task->pid, task_time);
...
printk(KERN_INFO "The total elapsed time of all processes for UID %d is %ld", UID, total_task_time);
